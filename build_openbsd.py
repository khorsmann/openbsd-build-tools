...
def build_release(release_base, site_base, force, interactive):
    ...
    run_command("/sbin/chown build %s" % os.environ['DESTDIR'])
    run_command("/bin/chmod 700 %s" % os.environ['DESTDIR'])
    run_command("/bin/mkdir -p %s" % os.environ['RELEASEDIR'])
    run_command("/sbin/chown build:wobj %s" % os.environ['RELEASEDIR'])
    run_command("/bin/chmod u=rwx %s" % os.environ['RELEASEDIR'])
